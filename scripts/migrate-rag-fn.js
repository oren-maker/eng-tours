...
(async () => {
  ...
  await c.connect();
  await c.query(fs.readFileSync('supabase/migrations/20260419_rag_match_fn.sql', 'utf8'));
  const r = await c.query(`SELECT proname FROM pg_proc WHERE proname='rag_match_documents'`);
  console.log('functions:', r.rows.map(x => x.proname).join(', '));
  await c.end();
})();
