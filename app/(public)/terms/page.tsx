...
export default async function TermsPage() {
  ...
  const { data } = await supabase.from("legal_documents").select("title, content, updated_at").eq("slug", "terms").single();
  const doc: any = data;
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-white rounded-xl shadow-sm p-8" dir="rtl">
          {doc ? (
            ...
            <p className="text-gray-500 text-center py-12">תנאי השימוש לא נמצאו</p>
          )}
        </div>
      </main>
    </div>
