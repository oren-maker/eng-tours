...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!ALLOWED.has(file.type)) return NextResponse.json({ error: "סוג קובץ לא נתמך" }, { status: 400 });

  const { data: page } = await supabase.from("marketing_pages").select("id, hero_image_url").eq("id", params.id).maybeSingle();
  if (!page) return NextResponse.json({ error: "לא נמצא" }, { status: 404 });

  const ext = file.name.split(".").pop()?.toLowerCase() || file.type.split("/")[1] || "bin";
  const key = `${params.id}/hero-${Date.now()}.${ext}`;
  ...
  const hero_image_url = pub?.publicUrl || null;

  if (page.hero_image_url) {
    const m = page.hero_image_url.match(/marketing-covers\/(.+)$/);
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  const supabase = createServiceClient();
  const { data: page } = await supabase.from("marketing_pages").select("hero_image_url").eq("id", params.id).maybeSingle();
  if (page?.hero_image_url) {
    const m = page.hero_image_url.match(/marketing-covers\/(.+)$/);
    if (m?.[1]) await supabase.storage.from("marketing-covers").remove([m[1]]).catch(() => {});
  }
  await supabase.from("marketing_pages").update({ hero_image_url: null, updated_at: new Date().toISOString() }).eq("id", params.id);
