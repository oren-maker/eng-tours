...
export default function IsraeliIdTestPage() {
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <h2 className="text-2xl font-bold text-primary-900">זיהוי תעודת זהות ישראלית</h2>
        <p className="text-sm text-gray-500 mt-1">
          העלה תמונת תעודת זהות. המערכת תזהה אם זו באמת ת״ז ישראלית, תאמת את ספרת הביקורת, ותחלץ את כל השדות.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <label
            htmlFor="idfile"
            className="flex flex-col items-center justify-center border-2 border-dashed border-primary-300 rounded-xl p-8 cursor-pointer hover:bg-primary-50 transition-colors"
          >
            <span className="text-4xl mb-2">📇</span>
            <span className="text-sm text-gray-600 mb-1">לחץ או גרור תמונה לכאן</span>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          {loading && (
            <div className="text-center py-12">
              ...
              <div className="text-sm text-gray-500 mt-3">שולח ל-Claude Vision API...</div>
            </div>
          )}
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className={`rounded-lg p-3 border-2 ${result.verified ? "bg-green-50 border-green-300" : "bg-red-50 border-red-300"}`}>
                ...
                <div className="text-xs mt-1 text-gray-700">
                  ...
                  {result.document_type_guess && <> · ניחוש סוג: <b>{result.document_type_guess}</b></>}
                </div>
                {result.checksum_valid !== null && (
                  <div className="text-xs mt-1">
                    ספרת ביקורת: <b className={result.checksum_valid ? "text-green-700" : "text-red-700"}>{result.checksum_valid ? "✓ תקינה" : "✗ לא תקינה"}</b>
                  </div>
                )}
              ...
              {result.data && (
                <div className="border border-gray-200 rounded-lg overflow-hidden">
                  <div className="bg-gray-50 px-3 py-2 text-xs font-semibold text-gray-700">שדות שחולצו</div>
                  <table className="w-full text-xs">
                    <tbody className="divide-y divide-gray-100">
                      {Object.entries(result.data).map(([k, v]) => (
                        <tr key={k} className={v ? "" : "opacity-50"}>
                          <td className="px-3 py-1.5 bg-gray-50 font-medium text-gray-600 w-36">{k}</td>
      ...
      <div className="mt-6 bg-blue-50 border border-blue-200 rounded-xl p-4 text-sm text-blue-800">
        ...
        <ul className="list-disc list-inside text-xs space-y-0.5">
          <li>המערכת משתמשת ב-Claude Vision API (Anthropic) לזיהוי וחילוץ.</li>
          <li>ספרת הביקורת של מספר תעודת הזהות מאומתת בצד שרת (אלגוריתם משרד הפנים).</li>
          <li>דורש <code>ANTHROPIC_API_KEY</code> בהגדרות Vercel.</li>
